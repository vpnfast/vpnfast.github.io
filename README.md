...
最初，市面上出现的VPN软件几乎是都是企业使用的，其作用就是帮助企业加密数据资料，确保信息的安全传输。后来才逐渐被中国及其他被墙地区的用户使用，他们通常会使用其核心的加密通信技术，伪装当前的通信流量，再利用**国外的VPN服务器**中转当前网络流量从而实现翻墙以及科学上网。由于VPN软件具备隐私保护和安全加密的优势，这让这些产品日益受到重视。但是，并不是所有的VPN梯子都能够成功绕开GFW（防火长城）的封控。其中，**安卓Google Play 和 App Store**可以说是重灾区，充斥着各种不稳定甚至毫无隐私保护的无效VPN工具，这类垃圾VPN软件既恶意插件泛滥的问题，又面临随时跑路的风险。简单的来说，**安全性和稳定性才是选择中国可用VPN时应关注的重点**。
